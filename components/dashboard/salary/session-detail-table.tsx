...
export function SessionDetailTable({ ptId, ptName, month, year, canEdit }: Props) {
  ...
  const totalThisMonth = rows.reduce((s, r) => s + r.sessionsThisMonth, 0);
    ...
    <>
      <div className="bg-[#fdf8f8] border border-[#f15b5c]/10 rounded-xl overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="py-6 text-center text-xs text-gray-400 italic">Không có hợp đồng ACTIVE nào</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse">
              ...
              <tbody>
                {rows.map(row => (
                  <tr key={row.enrollmentId} className="border-b border-gray-50 hover:bg-white/80 divide-x divide-gray-50">
                    ...
                    <td className={TD}>
                      <span className={cn(
                        ...
                        ["L1","L2","Loyalfit"].includes(row.packageName)
                    ...
                    <td className={cn(TD, "text-center font-semibold whitespace-nowrap",
                      row.sessionsRemaining <= 5 ? "text-red-500" : "text-gray-600")}>
                      {row.sessionsRemaining}
                    </td>
                    <td className={cn(TD, "text-center font-bold whitespace-nowrap",
                      row.sessionsThisMonth > 0 ? "text-gray-800" : "text-gray-400")}>
                    ...
                    <td className={cn(TD, "font-semibold whitespace-nowrap",
                      row.totalValue > 0 ? "text-gray-800" : "text-gray-400")}>
                    ...
                    <td className={TD}>
                      <CheckinCell
                        images={row.photo?.checkinImages ?? []}
                        canEdit={canEdit}
                        onView={idx => setLightbox({ images: row.photo!.checkinImages, index: idx })}
                        onUpload={() => openUpload(row, "checkin")}
                      />
                    </td>
                    ...
                    <td className={TD}>
                      <TransformCell
                        ...
                        canEdit={canEdit}
                        onToggle={() => toggleTransform(row)}
                        onView={idx => setLightbox({ images: row.photo!.transformImages, index: idx })}
                        onUpload={() => openUpload(row, "transform")}
                      />
                    </td>
                  </tr>
                ))}
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              ...
              <button onClick={() => setUploading(null)} className="text-gray-400 hover:text-gray-600">
                <X className="w-4 h-4" />
              </button>
            </div>
              ...
              {uploading.images.length < (uploading.type === "checkin" ? 2 : 3) && (
                ...
                >
                  ...
                  <p className="text-[10px] text-gray-400 mt-0.5">
                    Tối đa {uploading.type === "checkin" ? 2 : 3} ảnh
                    ({(uploading.type === "checkin" ? 2 : 3) - uploading.images.length} còn lại)
                  </p>
                  <input
                    ref={fileRef}
                    type="file"
            ...
            <div className="flex justify-end gap-3 px-5 py-4 border-t border-gray-100">
              ...
              <button
                ...
                disabled={saving}
                className="px-5 py-2 rounded-xl text-white text-sm font-bold disabled:opacity-60"
...
function CheckinCell({ images, canEdit, onView, onUpload }: {
  ...
  if (images.length === 0) {
    ...
    return (
      <button
        onClick={onUpload}
        className="flex items-center gap-1 text-gray-400 hover:text-[#f15b5c] transition-colors text-[10px] font-semibold whitespace-nowrap"
      >
    ...
    <div className="flex items-center gap-1">
      {images.map((img, i) => (
        <button
          key={i}
          onClick={() => onView(i)}
          className="w-12 h-12 rounded-lg overflow-hidden border border-gray-200 hover:border-[#f15b5c] transition-colors flex-shrink-0"
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={img} alt="" className="w-full h-full object-cover" />
        </button>
        ...
        <div className="flex items-center gap-1">
          {transformImages.map((img, i) => (
            <button
              key={i}
              onClick={() => onView(i)}
              className="w-12 h-12 rounded-lg overflow-hidden border border-green-200 hover:border-green-500 transition-colors flex-shrink-0"
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={img} alt="" className="w-full h-full object-cover" />
            </button>
